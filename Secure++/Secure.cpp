...
namespace Secure {
	...
	std::map<std::string, std::pair<std::string, std::string>> alphabets = {
		...
		{"rus" , {"‡·‚„‰Â∏ÊÁËÈÍÎÏÌÓÔÒÚÛÙıˆ˜¯˘˙˚¸˝˛ˇ", "¿¡¬√ƒ≈®∆«»… ÀÃÕŒœ–—“”‘’÷◊ÿŸ⁄€‹›ﬁﬂ"}}
	};
	...
	int index(char s1, std::string s2) { // ‚ÓÁ‚. ËÌ‰ÂÍÒ ÔÂ‚Ó„Ó ‚ıÓÊ‰ÂÌËˇ s1 ‚ s2
	...
	std::string Òaesar—rypt(std::string text, int key, std::string localisation) {
		std::string alphabet;
